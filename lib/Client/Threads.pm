...
sub show_threads {
    ...
    my $messages_api_url = Config::get_value_for("api_url") . "/messages/threads";
    ...
    if ( $rc >= 400 and $rc < 500 ) {
        ...
        _display_threads_stream($json->{threads}, $page_num, $json->{next_link_bool});
    } else  {
        Page->report_error("user", "Unable to complete request.", "Invalid response code returned from API.");
    }
}

sub _display_threads_stream {
    my $messages = shift;
    my $page_num = shift;
    my $next_link_bool = shift;

    my $t = Page->new("threads");
    ...
    my $next_page_url     = "/threads/$next_page_num";
    my $previous_page_url = "/threads/$previous_page_num";
